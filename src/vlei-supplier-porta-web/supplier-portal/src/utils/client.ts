...
export class ExtensionClient {
  ...
  private extensionIdPromise: Deferred<string | false> = new Deferred<string | false>();
  private options: ExtensionClientOptions;
  ...
  private handleEvent = async (event: MessageEvent) => {
  ...
  /**
   * Sends a /signify/credential/add-pending message to the extension.
   *
   * The extension should prompt the user to approve adding the pending credential.
   *
   * @param issuerOOBI  credential's issuer OOBI.
   * @returns {}
   */
  addPendingCredential = async (issuerOOBI: string): Promise<void> => {
    return this.sendMessage("/signify/credential/add-pending", { payload: { id: issuerOOBI } });
  };
